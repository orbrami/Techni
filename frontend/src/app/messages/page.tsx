...
import { Send, ArrowRight, Search, MessageCircle, Circle } from 'lucide-react';
...
interface Conversation {
  _id: string;
  participants: Array<{ _id: string; firstName: string; lastName: string; profilePicture?: string; grade: string; lastSeen: string }>;
...
export default function MessagesPage() {
  ...
  useEffect(() => {
    ...
    socket.on('new_message', ({ message, conversationId }: any) => {
    ...
    socket.on('typing_start', ({ userId }: any) => {
      setTypingUsers(prev => new Set([...prev, userId]));
    });

    socket.on('typing_stop', ({ userId }: any) => {
  ...
  // Handle ?with= param (direct message link)
  useEffect(() => {
    ...
    try {
      ...
      setConversations(prev => {
        const updated = prev.map(c => {
          ...
          if (otherParticipant?._id === activeConvUserId) {
            return { ...c, lastMessage: res.data.message, lastMessageAt: new Date().toISOString() };
          }
          return c;
        });
        return updated;
  ...
  const isTyping = (userId: string) => typingUsers.has(userId);
